/**
 * NURA PULSE - Payment Reconciliation Google AppScript V2
 * Updated to match actual payment screenshot extraction data structure
 * 
 * SETUP INSTRUCTIONS:
 ...
 * Column Order (matches backend sync payload):
 * A: Record ID
 * B: Driver Name
 * C: Vehicle Number
 * D: Platform (Rapido/Uber/Ola/Nura/Adhoc) - **NEW POSITION**
 * E: Date
 * F: Time
 * G: Description
 * H: Amount
...
// Column mapping (1-based indexing) - UPDATED with Platform in correct position
const COLUMNS = {
  ID: 1,              // A
  DRIVER: 2,          // B
  VEHICLE: 3,         // C
  PLATFORM: 4,        // D - **BETWEEN VEHICLE AND DATE**
  DATE: 5,            // E
  TIME: 6,            // F
  DESCRIPTION: 7,     // G
  AMOUNT: 8,          // H
...
function doGet(e) {
  return ContentService.createTextOutput(
    JSON.stringify({
      success: true,
      message: "Payment Reconciliation V2 sync endpoint is active",
...
/**
 * Helper: Ensure headers exist (with Platform between Vehicle and Date)
 */
function ensureHeaders(sheet) {
  ...
  if (!headers[0]) {
    const headerRow = [
      ...
      'Driver',           // B
      'Vehicle Number',   // C
      'Platform',         // D - **BETWEEN VEHICLE AND DATE**
      'Date',             // E
      'Time',             // F
      'Description',      // G
      'Amount',           // H
    ...
    Logger.log('Headers created with Platform in column D');
  }
}
...
function updateRow(sheet, row, record) {
  const values = [
    ...
    record.driver || 'N/A',
    record.vehicle || 'N/A',
    record.platform || 'N/A',        // Column D - **PLATFORM**
    record.date || 'N/A',
    record.time || 'N/A',
...
function testSync() {
  const testRecord = {
    ...
    driver: 'Test Driver',
    vehicle: 'TN01AB1234',
    platform: 'Rapido',  // Platform field included
    date: '15/11/2025',
    time: '10:30 AM',
    description: 'Auto',
    amount: '150',
