...
// Platform column after Driver Name
...
function doPost(e) {
  try {
    ...
    var monthYear = data.month_year || "Unknown";
    ...
    return createResponse(true, "Synced " + result.created + " new records", result);
...
function doGet(e) {
  var response = {
    success: true,
    message: "Payment Reconciliation sync endpoint active",
  ...
  try {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      try {
        ...
        errors.push("Error processing record: " + err.toString());
...
function initializeSheetForMonth() {
  var monthYear = "Nov 2025";
  var sheet = getOrCreateSheet(monthYear);
  ensureHeaders(sheet);
  Logger.log("Sheet initialized for " + monthYear);
}
