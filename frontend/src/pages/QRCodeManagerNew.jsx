...
const QRCodeManagerNew = () => {
  ...
  const createBatch = async (qrNamesArray) => {
    ...
    const payload = {
      ...
      landing_page_ios: iosUrl || null,
      landing_page_android: androidUrl || null,
      landing_page_mobile: webUrl || null,
      landing_page_desktop: webUrl || null,
  ...
  const handleDeleteSelectedCampaigns = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedCampaigns.length} campaign(s)? This will delete all QR codes in these campaigns, including published ones.`)) {
      return;
    }
    ...
    try {
      ...
      // Delete each campaign with force=true to delete published campaigns
      for (const campaignName of selectedCampaigns) {
        try {
          await axios.delete(`${API}/qr-codes/campaigns/${encodeURIComponent(campaignName)}?force=true`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          successCount++;
          ...
          failCount++;
          console.error(`✗ Failed to delete campaign ${campaignName}:`, error.response?.data?.detail || error.message);
        }
      }
      
      if (successCount > 0) {
        toast.success(`Successfully deleted ${successCount} campaign(s)${failCount > 0 ? `. Failed to delete ${failCount}.` : ''}`);
      } else {
        toast.error(`Failed to delete all ${failCount} campaigns. You may need master admin permissions.`);
      }
  ...
  const handleDeleteAllQRCodes = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ALL ${campaignQRCodes.length} QR codes in this campaign? This will delete them even if published.`)) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem("token");
      // Delete all QR codes with force=true
      for (const qrCode of campaignQRCodes) {
        try {
          await axios.delete(`${API}/qr-codes/${qrCode.id}?force=true`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          successCount++;
        } catch (error) {
          failCount++;
          console.error(`Failed to delete QR ${qrCode.id}:`, error.response?.data?.detail || error.message);
        }
      }
