from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, Query, Request, Form, UploadFile, File, BackgroundTasks
...
async def get_share_link(file_id: str, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        backend_url = os.environ.get('BACKEND_URL', 'https://hotspot-manager-10.preview.emergentagent.com')
...
async def analyze_hotspot_placement(
    ...
    try:
        ...
        # Find time column
        time_col = None
        for col in ['time', 'datetime', 'createdAt', 'rideAssignedTime', 'pickup_time', 'request_time', 'updatedAt']:
            if col in df_clean.columns:
                time_col = col
                logger.info(f"Using time column: {col}")
                break
        
        if not time_col:
            raise HTTPException(
                status_code=400,
                detail="No time column found. Expected 'time', 'datetime', 'createdAt', 'rideAssignedTime', 'pickup_time', 'request_time', or 'updatedAt'"
            )
...
async def create_qr_code(
    ...
    try:
        ...
        # Get backend URL from environment
        backend_url = os.environ.get('BACKEND_URL', 'https://hotspot-manager-10.preview.emergentagent.com/api')
