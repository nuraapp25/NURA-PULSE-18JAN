...
2024-12-15,18:15:00,13.0327,80.2007,13.0377,80.2085"""
    ...
    def test_telecaller_desk_enhancements(self):
        """Test Telecaller's Desk Backend Enhancements - Callback Date Calculation, Lead Sorting, and Summary"""
        ...
        if response and response.status_code == 200:
            try:
                ...
                if leads and len(leads) > 0:
                    test_lead_id = leads[0].get("id")
                    self.log_test("Telecaller Desk - Get Test Lead", True, 
                                f"Retrieved test lead ID: {test_lead_id}")
                    success_count += 1
                else:
                    self.log_test("Telecaller Desk - Get Test Lead", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Telecaller Desk - Get Test Lead", False, error_msg)
            return False
        
        if not test_lead_id:
            self.log_test("Telecaller Desk - Test Setup", False, 
                        "Cannot proceed without test lead ID")
            return False
        
        # Test 2: Lead Update with Callback Status - Test all 4 callback options
        print("\n--- Test 2: Lead Update with Callback Status Testing ---")
            ...
            if response and response.status_code == 200:
                try:
                    ...
                    if "message" in result and "successfully" in result["message"].lower():
                        self.log_test(f"Callback Status - {status}", True, 
                                    f"Successfully updated lead to {status}")
        ...
        if response and response.status_code == 200:
            # Verify callback_date is cleared
            lead_response = self.make_request("GET", f"/driver-onboarding/leads/{test_lead_id}")
            if lead_response and lead_response.status_code == 200:
                try:
                    lead_data = lead_response.json()
                    ...
                    if callback_date is None:
                        self.log_test("Callback Date - Clear for Non-Callback", True, 
                                    "Callback date correctly cleared for non-callback status")
                        success_count += 1
                    else:
                        self.log_test("Callback Date - Clear for Non-Callback", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Callback Date - Clear for Non-Callback", False, error_msg)
        
        # Test 4: Lead Sorting Logic - Test that uncalled leads appear first
        print("\n--- Test 4: Lead Sorting Logic Testing ---")
            ...
            try:
                ...
                if uncalled_leads:
                    ...
                    if call_response and call_response.status_code == 200:
                        self.log_test("Lead Sorting - Mark Call Done", True, 
                        ...
                        if response_after and response_after.status_code == 200:
                            try:
                                leads_after = response_after.json()
                        ...
                        self.log_test("Lead Sorting - Mark Call Done", False, error_msg)
                else:
                    self.log_test("Lead Sorting - Mark Call Done", True, 
        ...
        # Test 7: Verify callback_date is returned in leads list
        print("\n--- Test 7: Callback Date in Leads List ---")
        ...
        if response and response.status_code == 200:
            try:
                ...
                # Check if any leads have callback_date field
                leads_with_callback = [lead for lead in leads if lead.get("callback_date")]
                
                if leads_with_callback:
                    self.log_test("Callback Date - In Leads List", True, 
                                f"Found {len(leads_with_callback)} leads with callback_date in list")
                    success_count += 1
                else:
                    # Check if our test lead is in the list and has callback_date
                    test_lead_in_list = next((lead for lead in leads if lead.get("id") == test_lead_id), None)
                    if test_lead_in_list and test_lead_in_list.get("callback_date"):
                        self.log_test("Callback Date - In Leads List", True, 
                                    "Test lead has callback_date in leads list")
                        success_count += 1
                    else:
                        self.log_test("Callback Date - In Leads List", False, 
                                    "No leads with callback_date found in list")
            except Exception as e:
                self.log_test("Callback Date - In Leads List", False, 
                            f"Error checking leads list: {e}")
        else:
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Callback Date - In Leads List", False, error_msg)
    ...
    def test_vehicle_documents_comprehensive(self):
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if result.get("success") and "file_path" in result:
                        ...
                        self.log_test("Vehicle Documents - File Upload", True, 
                                    f"File uploaded successfully: {result['filename']}, path: {test_file_path}")
                        success_count += 1
                    else:
                        self.log_test("Vehicle Documents - File Upload", False, 
            ...
            self.log_test("Vehicle Documents - File Upload", False, f"Exception during upload: {e}")
        ...
        if response and response.status_code == 200:
            try:
                ...
                if result.get("success") and "document_id" in result:
                    ...
                    self.log_test("Vehicle Documents - Create Document", True, 
                                f"Document created successfully: ID {test_document_id}")
                    success_count += 1
                else:
                    self.log_test("Vehicle Documents - Create Document", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Vehicle Documents - Create Document", False, error_msg, 
                        response.text if response else None)
            ...
            if response and response.status_code == 200:
                try:
                    ...
                    if result.get("success") and "document" in result:
                        ...
                        self.log_test("Vehicle Documents - Get Single Document", True, 
                                    f"Retrieved document: VIN {document.get('vin')}, Vehicle: {document.get('vehicle_name')}")
                        success_count += 1
                    else:
                        self.log_test("Vehicle Documents - Get Single Document", False, 
                ...
                error_msg = "Network error" if not response else f"Status {response.status_code}"
                self.log_test("Vehicle Documents - Get Single Document", False, error_msg)
        else:
            self.log_test("Vehicle Documents - Get Single Document", False, 
                        "No test document ID available")
        
        # Test 4: Get Single Document - 404 Test
        print("\n--- Testing Get Single Document - 404 ---")
        response = self.make_request("GET", "/vehicle-documents/nonexistent-id-12345")
        
        if response and response.status_code == 404:
            self.log_test("Vehicle Documents - Get Document 404", True, 
                        "Correctly returns 404 for non-existent document")
            success_count += 1
        else:
            status = response.status_code if response else "Network error"
            self.log_test("Vehicle Documents - Get Document 404", False, 
                        f"Expected 404, got {status}")
        
        # Test 5: List All Documents with Pagination
        print("\n--- Testing List All Documents ---")
        ...
        if response and response.status_code == 200:
            try:
                ...
                if result.get("success") and "documents" in result and "total" in result:
                    ...
                    total = result["total"]
                    self.log_test("Vehicle Documents - List Documents", True, 
                                f"Retrieved {len(documents)} documents, total: {total}")
                    success_count += 1
                else:
                    self.log_test("Vehicle Documents - List Documents", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Vehicle Documents - List Documents", False, error_msg)
        ...
        if response and response.status_code == 200:
            try:
                ...
                if result.get("success"):
                    ...
                    if found_test_doc:
                        self.log_test("Vehicle Documents - Search Functionality", True, 
                                    f"Search found test document in {len(documents)} results")
                        success_count += 1
                    else:
                        self.log_test("Vehicle Documents - Search Functionality", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Vehicle Documents - Search Functionality", False, error_msg)
        ...
        if test_document_id:
            ...
            if response and response.status_code == 200:
                try:
                    ...
                    if result.get("success") and "document" in result:
                        ...
                        if updated_doc.get("vehicle_cost") == 550000:
                            self.log_test("Vehicle Documents - Update Document", True, 
                                        f"Document updated successfully: cost now {updated_doc.get('vehicle_cost')}")
                            success_count += 1
                        else:
                            self.log_test("Vehicle Documents - Update Document", False, 
                ...
                error_msg = "Network error" if not response else f"Status {response.status_code}"
                self.log_test("Vehicle Documents - Update Document", False, error_msg)
        else:
            self.log_test("Vehicle Documents - Update Document", False, 
                        "No test document ID available")
        
        # Test 8: Update Document - 404 Test
        print("\n--- Testing Update Document - 404 ---")
        update_data = {"vehicle_cost": 600000}
        response = self.make_request("PUT", "/vehicle-documents/nonexistent-id-12345", update_data)
        
        if response and response.status_code == 404:
            self.log_test("Vehicle Documents - Update Document 404", True, 
                        "Correctly returns 404 for non-existent document update")
            success_count += 1
        else:
            status = response.status_code if response else "Network error"
            self.log_test("Vehicle Documents - Update Document 404", False, 
                        f"Expected 404, got {status}")
        ...
        if response and response.status_code == 200:
            try:
                result = response.json()
                if result.get("success"):
                    self.log_test("Vehicle Documents - Date Field Parsing", True, 
                                "Successfully parsed various ISO date formats")
                    success_count += 1
                else:
                    self.log_test("Vehicle Documents - Date Field Parsing", False, 
            ...
            error_msg = "Network error" if not response else f"Status {response.status_code}"
            self.log_test("Vehicle Documents - Date Field Parsing", False, error_msg)
        
        # Test 10: Get Document File
        print("\n--- Testing Get Document File ---")
        if test_file_path:
            ...
            if response and response.status_code == 200:
                ...
                if 'application/pdf' in content_type or len(response.content) > 0:
                    self.log_test("Vehicle Documents - Get Document File", True, 
                                f"File retrieved successfully: {len(response.content)} bytes, type: {content_type}")
                    success_count += 1
                else:
                    self.log_test("Vehicle Documents - Get Document File", False, 
                                f"Invalid file response: {content_type}")
            else:
                error_msg = "Network error" if not response else f"Status {response.status_code}"
                self.log_test("Vehicle Documents - Get Document File", False, error_msg)
        else:
            self.log_test("Vehicle Documents - Get Document File", False, 
                        "No test file path available")
        
        # Test 11: Get Document File - 404 Test
        print("\n--- Testing Get Document File - 404 ---")
        response = self.make_request("GET", "/vehicle-documents/file/nonexistent/file.pdf")
        
        if response and response.status_code == 404:
            self.log_test("Vehicle Documents - Get File 404", True, 
                        "Correctly returns 404 for non-existent file")
            success_count += 1
        else:
            status = response.status_code if response else "Network error"
            self.log_test("Vehicle Documents - Get File 404", False, 
                        f"Expected 404, got {status}")
        ...
        endpoints_to_test = [
            ...
            ("POST", "/vehicle-documents/upload-file")
        ]
        
        for method, endpoint in endpoints_to_test:
            try:
                if method == "POST" and "upload-file" in endpoint:
                    # Skip file upload test without auth as it's complex
                    continue
        ...
        if test_document_id:
            ...
            if response and response.status_code == 200:
                try:
                    result = response.json()
                    if result.get("success"):
                        self.log_test("Vehicle Documents - Delete Permission (Admin)", True, 
                                    "Admin successfully deleted document")
                        success_count += 1
                    else:
                        self.log_test("Vehicle Documents - Delete Permission (Admin)", False, 
                ...
                error_msg = "Network error" if not response else f"Status {response.status_code}"
                self.log_test("Vehicle Documents - Delete Permission (Admin)", False, error_msg)
        else:
            self.log_test("Vehicle Documents - Delete Permission (Admin)", False, 
                        "No test document ID available for delete test")
        
        # Test 14: Delete Document - 404 Test
        print("\n--- Testing Delete Document - 404 ---")
        response = self.make_request("DELETE", "/vehicle-documents/nonexistent-id-12345")
        
        if response and response.status_code == 404:
            self.log_test("Vehicle Documents - Delete Document 404", True, 
                        "Correctly returns 404 for non-existent document delete")
            success_count += 1
        else:
            status = response.status_code if response else "Network error"
            self.log_test("Vehicle Documents - Delete Document 404", False, 
                        f"Expected 404, got {status}")
    ...
    def run_all_tests(self):
        ...
        critical_tests = [
            self.test_qr_code_campaign_delete_fix,        # CRITICAL: QR Code Campaign Delete Fix - Scan Cleanup (CURRENT REVIEW)
